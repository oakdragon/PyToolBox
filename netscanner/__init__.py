...
try:
    ...
    min_port = 0  # raw_input("[*] Enter Minumum Port Number: ") # Get Min. Port Num.
    max_port = 1025  # raw_input("[*] Enter Maximum Port Number: ") # Get Max. Port Num.
    try:
        if int(min_port) >= 0 and int(max_port) >= 0 and int(max_port) >= int(
                min_port):  # Test for valid range of ports
            pass
...
def checkhost(ip):  # Function to check if target is up
    ...
    try:
        ping = sr1(IP(dst=ip) / UPD())  # Ping the target
...
for port in ports:  # Iterate through range of ports
    ...
    if status is True:  # Test result // changed == to is
